...
class GenericAssistant(IAssistant):
    ...
    def save_model(self, model_name=None):
        if model_name is None:
            self.model.save(f"botModel/{self.model_name}.h5", self.hist)
            pickle.dump(self.words, open(f'botModel/{self.model_name}_words.pkl', 'wb'))
            pickle.dump(self.classes, open(f'botModel/{self.model_name}_classes.pkl', 'wb'))
        else:
            self.model.save(f"botModel/{model_name}.h5", self.hist)
            pickle.dump(self.words, open(f'botModel/{model_name}_words.pkl', 'wb'))
            pickle.dump(self.classes, open(f'botModel/{model_name}_classes.pkl', 'wb'))

    def load_model(self, model_name=None):
        if model_name is None:
            self.words = pickle.load(open(f'botModel/{self.model_name}_words.pkl', 'rb'))
            self.classes = pickle.load(open(f'botModel/{self.model_name}_classes.pkl', 'rb'))
            self.model = load_model(f'botModel/{self.model_name}.h5')
        else:
            self.words = pickle.load(open(f'botModel/{model_name}_words.pkl', 'rb'))
            self.classes = pickle.load(open(f'botModel/{model_name}_classes.pkl', 'rb'))
            self.model = load_model(f'botModel/{model_name}.h5')
